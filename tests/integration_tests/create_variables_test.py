...
def test_numeric_var(mocker):
    raw_numeric_var = mocker.Mock(type="Numeric", to_dict=mocker.Mock(return_value={}))
...
def test_array_var(mocker):
    raw_array_var = mocker.Mock(
        ...
        to_dict=mocker.Mock(
            return_value={
                "selector_info": {"sub_type": "Categorical", "selector_type": "OrArray"}
            }
        ),
    )
...
def test_date_var(mocker):
    raw_date_var = mocker.Mock(
        type="Selector",
        selector_info=mocker.Mock(sub_type="Date"),
        to_dict=mocker.Mock(
            return_value={
                "selector_info": {"sub_type": "Date", "selector_type": "SingleValue"}
            }
        ),
    )
...
def test_datetime_var(mocker):
    raw_datetime_var = mocker.Mock(
        type="Selector",
        selector_info=mocker.Mock(sub_type="DateTime"),
        to_dict=mocker.Mock(
            return_value={
                "selector_info": {
                    "sub_type": "DateTime",
    ...
    assert IVA._choose_variable(raw_datetime_var) == DateTimeVariable
